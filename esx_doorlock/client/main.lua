...
--Function for auto doorlock
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(2500) --I set this to 2.5seconds so when the door swings it will have some time before locking
		local playerCoords = GetEntityCoords(PlayerPedId())
		...
		for i=1, #Config.DoorList do
			...
			if distance > maxDistance then
				...
				if doorID.autolock then
					TriggerServerEvent('esx_doorlock:updateState', i, true) --Force to true when the player distance is far away from the door distance
				end
			end
		end
	end
...
function ApplyDoorState(doorID)
	local closeDoor = GetClosestObjectOfType(doorID.objCoords.x, doorID.objCoords.y, doorID.objCoords.z, 1.0, doorID.HashKey, false, false, false) --I use hashkey instead of the object namne because there some doors that i cannot get the name
